...
import type { CertificatesDocument } from '../types/schema';
...
export async function getCertificatesUrl(): Promise<string[]> {
  ...
  return [
    `${process.env.NEXT_PUBLIC_URL_BASE}/certificates`,
    ...d.results.map(
      (x) => `${process.env.NEXT_PUBLIC_URL_BASE}/certificates/${x.uid}`,
    ),
  ];
}
...
export async function getCertificatesBySlug({
  slug,
}: querySlug): Promise<CertificatesDocument<string>> {

...
function GraphGrid(props: { year: number; activity: Activity[] }) {
  ...
  const weeksActivity = weeks.map((x) => {
    ...
    return days.map((x) => {
      return atvMap.has(x.toISOString().split("T")[0])
        ? atvMap.get(x.toISOString().split("T")[0])
        : undefined;
    });
  });
    ...
    <>
      <div className="grid grid-cols-12 gap-2 py-2">
        {labelsData.months.map((x, i) => (
          <div key={`graph-label-${i}`} className="text-xs text-zinc-500">
            {x}
          </div>
        ))}
      </div>
  ...
  return (
    <article className="flex flex-col gap-y-2 text-sm">
      ...
      <footer className="flex justify-between items-center">
        <div>
          {data.length} contribuciones en {currentYear}
        </div>
...
export function ContributionGraph() {
  ...
  const dataYear = data?.contributions.filter((contribution) =>
    contribution.date.startsWith(`${year}-`),
  );
